      ...
      <div className="download-banner__inner">
        <div className="download-banner__content" aria-label="download banner">
          <div className="download-banner__image rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 p-6 text-white">
            <Text typography="t6" fontWeight="medium" as="p" className="opacity-90">
              TradeFlow
            </Text>
            <Text typography="st5" fontWeight="bold" as="p" className="mt-2">
              모바일 앱으로
              <br />
              더 빠르게 거래하세요
            </Text>
          ...
          <div className="download-banner__promo flex items-center justify-between gap-3">
            <Text
              ...
              color="grey900"
              as="p"
              className="flex-1 pl-1 whitespace-pre-line"
            >
              앱 첫 설치 시{'\n'}수수료 50% 할인
            </Text>
            <div
            ...
            >
              <div
                ...
                className="grid h-[140px] w-[140px] grid-cols-5 grid-rows-5 gap-1 rounded-lg border border-grey-200 bg-white p-2"
              >
                {Array.from({ length: 25 }).map((_, i) => (
                  <div
                    key={i}
                    className={`rounded-sm ${i % 3 === 0 ? 'bg-grey-900' : 'bg-grey-100'}`}
                  />
                ))}
              </div>
            </div>
